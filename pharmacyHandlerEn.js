...
function intialize() {
    ...
    $("#administrative-areas").attr("disabled", true);
    $("#myInput").val("");
}

function changeKeyWordText() {
    $('#administrative-areas').change(function() {
...
function getAvailableMaskDataLists(maskDataUrl) {
    ...
    $.get(maskDataUrl, function(data) {
        ...
        for (let item in lines) {
            ...
            let addr = lines[item][2] ? lines[item][2] : "No Address";
            let tel = lines[item][3] ? lines[item][3] : "No phone number";
            ...
            let updatedTime = 'No latest updated time';
            ...
            pharmacyNumber = lines[item][1].replace('醫事機構名稱', 'Pharmacy number');

            //Add new card body
            let content = [
                "<div class='card-body'>",
                "<h5 class='card-title' ><p>", pharmacyId, "</p>", pharmacyNumber + "</h5>",
                "<p>",
                ...
                "</p>",
                "<a href = '", mapURL, "' target='_blank' class='card-link'>Google Map Link</a>",
                "<a href = 'http://www.nhi.gov.tw/QueryN/Query3_Detail.aspx?HospID=", lines[item][0].split(':')[1], "' target='_blank' class='card-link'>NHI Link</a>",
                "</div>"
            ].join("");
            $("#list").append(content);
            $("#loading").hide();
            ...
            $("#administrative-areas").attr("disabled", false);
        }
    });
}
